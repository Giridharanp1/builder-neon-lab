...
const mockSuppliers: Supplier[] = [
  {
    ...
    minOrder: 500,
    products: [
      { id: '1', name: 'Fresh Tomatoes', category: 'Vegetables', price: 45, unit: 'kg', inStock: true },
      { id: '2', name: 'Onions', category: 'Vegetables', price: 35, unit: 'kg', inStock: true },
      { id: '3', name: 'Green Chilies', category: 'Vegetables', price: 80, unit: 'kg', inStock: true }
    ],
    isPartner: true,
  ...
  },
  {
    ...
    description: 'Authentic spices and masalas directly from farms across India',
    specialties: ['Spices', 'Masalas', 'Dry Fruits', 'Herbs'],
    priceRange: '₹100-2000/kg',
    deliveryTime: '1-3 hours',
    minOrder: 300,
    products: [
      { id: '4', name: 'Red Chili Powder', category: 'Spices', price: 180, unit: 'kg', inStock: true },
      { id: '5', name: 'Turmeric Powder', category: 'Spices', price: 120, unit: 'kg', inStock: true },
      { id: '6', name: 'Garam Masala', category: 'Spices', price: 200, unit: 'kg', inStock: false }
    ],
    isPartner: true,
  ...
  },
  {
    ...
    minOrder: 200,
    products: [
      { id: '7', name: 'Fresh Milk', category: 'Dairy', price: 55, unit: 'liter', inStock: true },
      { id: '8', name: 'Paneer', category: 'Dairy', price: 280, unit: 'kg', inStock: true }
    ],
    isPartner: false,
  ...
  },
  {
    ...
    minOrder: 1000,
    products: [
      { id: '9', name: 'Basmati Rice', category: 'Grains', price: 120, unit: 'kg', inStock: true },
      { id: '10', name: 'Wheat Flour', category: 'Grains', price: 45, unit: 'kg', inStock: true }
    ],
    isPartner: false,
    totalOrders: 3
  }
...
export default function Suppliers() {
  ...
  const filteredSuppliers = suppliers.filter(supplier => {
    ...
    const matchesCategory = categoryFilter === 'all' || supplier.specialties.includes(categoryFilter);
        ...
        <header className="border-b bg-white/80 backdrop-blur-md sticky top-0 z-50">
          <div className="container mx-auto px-4 py-4 flex items-center justify-between">
            ...
            <nav className="hidden md:flex items-center space-x-6">
              ...
              <Link to="/suppliers" className="text-sm text-foreground font-medium">
                Suppliers
              </Link>
            </nav>
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                <CardContent className="p-6">
                  <div className="flex items-start space-x-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-muted-foreground mb-2">{selectedSupplier.businessType}</p>
                      <div className="flex items-center space-x-4 text-sm">
                        <div className="flex items-center space-x-1">
                          <Star className="h-4 w-4 fill-yellow-400 text-yellow-400" />
                          <span className="font-medium">{selectedSupplier.rating}</span>
                          <span className="text-muted-foreground">({selectedSupplier.reviews} reviews)</span>
                        </div>
                        <div className="flex items-center space-x-1">
                          <MapPin className="h-4 w-4 text-muted-foreground" />
                          <span>{selectedSupplier.location}</span>
                          <span className="text-muted-foreground">• {selectedSupplier.distance}</span>
                        </div>
                      </div>
                    </div>
                    <Button
                      variant={favorites.includes(selectedSupplier.id) ? "default" : "outline"}
                      size="sm"
                      onClick={() => toggleFavorite(selectedSupplier.id)}
                    >
                      <Heart className={`h-4 w-4 mr-2 ${favorites.includes(selectedSupplier.id) ? 'fill-current' : ''}`} />
                      {favorites.includes(selectedSupplier.id) ? 'Favorited' : 'Add to Favorites'}
                    </Button>
                  </div>
                  
                  <p className="mt-4 text-muted-foreground">{selectedSupplier.description}</p>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-6">
                    <div>
                      ...
                      <p className="font-medium">{selectedSupplier.priceRange}</p>
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground mb-1">Delivery Time</p>
                      <p className="font-medium">{selectedSupplier.deliveryTime}</p>
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground mb-1">Minimum Order</p>
                      <p className="font-medium">₹{selectedSupplier.minOrder.toLocaleString()}</p>
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground mb-1">Total Orders</p>
                      <p className="font-medium">{selectedSupplier.totalOrders} orders</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <Button className="w-full mt-4" onClick={() => viewProducts(selectedSupplier)}>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground">Find and manage your trusted supplier network</p>
          </div>
          <Button>
        ...
        <Tabs defaultValue="all" className="space-y-6">
                ...
                <Card key={supplier.id} className="hover:shadow-lg transition-shadow">
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between mb-4">
                      ...
                      >
                        <Heart className={`h-4 w-4 ${favorites.includes(supplier.id) ? 'fill-current text-red-500' : ''}`} />
                      </Button>
                    </div>
          ...
          <TabsContent value="favorites">
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {suppliers.filter(s => favorites.includes(s.id)).map((supplier) => (
                <Card key={supplier.id} className="hover:shadow-lg transition-shadow">
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center space-x-3">
          ...
          <TabsContent value="partners">
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {suppliers.filter(s => s.isPartner).map((supplier) => (
                <Card key={supplier.id} className="hover:shadow-lg transition-shadow border-purple-200">
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center space-x-3">
                        <Avatar>
